...
1. **Connect the repo via a GitHub App** (Sources → GitHub App). A plain deploy-key source cannot
   do Preview Deployments. Requires Coolify ≥ v4.0.0-beta.411.
2. **Create the resource**: New Resource → this repo → build pack **Docker Compose** → compose file
   `docker-compose.coolify.yml` → branch `development`.
3. **Set environment variables**: `CREDENTIALS_ENC_KEY` (`openssl rand -hex 32`, 64 hex chars),
   `COHERE_API_KEY`, `GITHUB_AUTH`, `SMTP_HOST`/`SMTP_PORT`/`SMTP_SECURE`/`SMTP_USER`/`SMTP_PASS`/
   `SMTP_FROM`, and optionally the `GITHUB_TOKEN` build arg. `SERVICE_PASSWORD_POSTGRES` /
   `SERVICE_PASSWORD_KCADMIN` are generated by Coolify.
4. **Enable Preview Deployments** on the resource. Coolify then builds a preview for each open PR
   and destroys it on close.

The API image runs pending migrations on boot, so each preview's fresh Postgres provisions itself.
