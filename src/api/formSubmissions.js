...
export const loadMyFormSubmissions = (region) =>
  get(`/regions/${region}/form-submissions/mine`);

export const createFormSubmission = (region, formTypeId, data = {}) =>
  post(`/regions/${region}/form-types/${formTypeId}/submissions`, { data });

export const getFormSubmission = (region, id) =>
  get(`/regions/${region}/form-submissions/${id}`);

export const saveFormSubmission = (region, id, data, status) =>
  put(`/regions/${region}/form-submissions/${id}`, {
    data,
    ...(status && { status }),
  });
