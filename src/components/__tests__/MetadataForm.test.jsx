...
describe('<MetadataForm />', () => {
  ...
  it('Handles input changes without a parent', () => {
    ...
    wrapper.find(BilingualTextInput).at(0).props().onChange({ target: {name: "Gandalf", value: "Narya"}});
  });

  it('Handles input changes with a parent', () => {
    const wrapper = mount(<MetadataForm />);

    wrapper.find(BilingualTextInput).at(0).props().onChange({ target: {name: "Gandalf", value: "Narya"}}, true);
  });
});
