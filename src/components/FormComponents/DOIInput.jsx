...
const DOIInput = ({
  ...
  const statusEditable =
    ...
    (!datasetIsDraft ||
      ["registered", "findable"].includes(record.doiCreationStatus));
  const showStatusSelect =
    isOurManagedDoi &&
    ["draft", "registered", "findable"].includes(record.doiCreationStatus);
  const statusOptions = DOI_STATE_TRANSITIONS[record.doiCreationStatus] || [
    record.doiCreationStatus,
  ];
  ...
  async function handleGenerateDOI() {
    ...
    try {
      ...
      if (selectedSuffixMode === "identifier") {
        ...
        if (!doiSuffix) {
          throw new Error(
            "Record identifier is empty; cannot use it as the DOI suffix.",
          );
        }
      ...
      console.log("[DOIInput] createDraftDoi payload", {
        region,
        minimalPayload,
      });
      ...
      await createDraftDoi({
        ...
        .then(async (attributes) => {
          ...
          handleUpdateDatasetIdentifier({
            target: { value: `https://doi.org/${attributes.doi}` },
          });
          ...
          if (["submitted", "published"].includes(record.status)) {
            try {
              ...
              console.error(
                "[DOIInput] auto-update after generate failed",
                updateErr,
              );
              setDoiErrorFlag(true);
              setDoiErrorMessage(
                updateErr.message ||
                  "DOI created but failed to push metadata. Try clicking Update DOI.",
              );
            }
          }
        })
      ...
      setDoiErrorFlag(true);
      const errorMessage =
        err.message || "Failed to generate DOI. Please try again.";
      setDoiErrorMessage(errorMessage);
      setLoadingDoi(false);
      console.error("[DOIInput] handleGenerateDOI failed", {
        code: err.code,
        message: err.message,
        details: err.details,
      });
  ...
  async function handleUpdateDraftDOI() {
    ...
    try {
      ...
      const errorMessage =
        err.message || "Failed to update DOI. Please try again.";
      setDoiErrorMessage(errorMessage);
  ...
  async function handleDeleteDOI() {
    ...
    try {
      ...
      setDoiErrorFlag(true);
      const errorMessage =
        err.message || "Failed to delete DOI. Please try again.";
      setDoiErrorMessage(errorMessage);
      setLoadingDoiDelete(false);
    }
  }
  ...
  async function handleDoiStateTransition(targetState) {
    ...
    try {
      ...
      if (record.recordID) {
        await saveRecord(region, record.recordID, {
          ...record,
      ...
      setDoiStateError(
        err.message || "Failed to update DOI status. Please try again.",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    if (debouncedDoiIdValue === "") {
      ...
      getDoiStatus({ doi: id, region })
        .then((response) => {
          if (mounted.current)
            handleUpdateDoiCreationStatus({
              target: { name, value: response.data },
            });
        })
        .catch((err) => {
          console.error(err);
    ...
    // handleUpdateDoiCreationStatus is deliberately left out: MetadataForm
    // creates it on every render, so depending on it re-runs this effect, which
    // sets state in the parent, which re-renders — an infinite loop.
  ...
  function getStatusConsequence(current, target) {
    if (target === "registered" && current === "findable") {
      return {
        ...
        bodyEn:
          "The DOI will be hidden from public discovery — it will be removed from DataCite search and metadata indexes. The DOI itself stays permanent and the landing page link keeps working; it just won't be publicly findable.",
        bodyFr:
          "Le DOI sera masqué de la découverte publique — il sera retiré de la recherche et des index de métadonnées de DataCite. Le DOI reste permanent et le lien de la page de destination continue de fonctionner ; il ne sera simplement plus repérable publiquement.",
      };
    }
    if (target === "registered") {
      return {
        ...
        bodyEn:
          "Registering makes the DOI permanent — it can no longer be deleted. It will not be publicly discoverable until you publish it (Findable).",
        bodyFr:
          "L'enregistrement rend le DOI permanent — il ne pourra plus être supprimé. Il ne sera pas repérable publiquement tant que vous ne l'aurez pas publié (Trouvable).",
      };
    }
    return {
      ...
      bodyEn:
        "The DOI and its metadata become publicly discoverable and indexed by DataCite. This is permanent — the DOI can no longer be deleted, only demoted back to Registered (hidden) later.",
      bodyFr:
        "Le DOI et ses métadonnées deviennent repérables publiquement et indexés par DataCite. Cette action est permanente — le DOI ne peut plus être supprimé, seulement rétrogradé à Enregistré (masqué) ultérieurement.",
    };
  }
  ...
  function requestDelete() {
    setConfirmAction({
      ...
      bodyEn:
        "This permanently deletes the draft DOI from DataCite and clears the DOI field. Only draft DOIs can be deleted — once a DOI is registered or findable it becomes permanent and cannot be removed.",
      bodyFr:
        "Cela supprime définitivement le brouillon de DOI de DataCite et vide le champ DOI. Seuls les brouillons de DOI peuvent être supprimés — une fois enregistré ou trouvable, un DOI devient permanent et ne peut plus être retiré.",
              ...
              >
                {statusOptions.map((s) => (
                  <MenuItem key={s} value={s}>
                    <I18n
                      en={DOI_STATE_LABELS[s].en}
                      fr={DOI_STATE_LABELS[s].fr}
                    />
                  </MenuItem>
                ))}
              </Select>
      ...
      {inGenerateStage && (
        <div style={{ marginTop: "16px" }}>
          <SupplementalText>
            <I18n>
              <En>
                Don&apos;t have a DOI yet? Reserve a draft DOI with DataCite.
                Please save the form first.
              </En>
              <Fr>
                Vous n&apos;avez pas encore de DOI ? Réservez un brouillon de
                DOI auprès de DataCite. Veuillez d&apos;abord enregistrer le
                formulaire.
              </Fr>
            </I18n>
          </SupplementalText>
              ...
              >
                {availableSuffixModes.includes("default") && (
                  <FormControlLabel
                    ...
                    control={<Radio />}
                    label={
                      <I18n en="Auto-generated" fr="Généré automatiquement" />
                    }
                  />
                )}
        ...
        <div style={{ marginTop: "16px" }}>
          ...
          >
            ...
            {showDataciteRecordButton && (
              ...
              >
                <I18n
                  en="View DataCite record"
                  fr="Voir l'enregistrement DataCite"
                />
              </Button>
            )}
            {showDeleteDoi && (
