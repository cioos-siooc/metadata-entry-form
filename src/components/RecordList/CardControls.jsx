...
const CardControls = () => {
  ...
  const [quick, setQuick] = useState(() => (filterModel.quickFilterValues?.[0] || ''));
  const [debouncedQuick] = useDebounce(quick, 300);
  const [author, setAuthor] = useState(() => {
    const item = (filterModel.items || []).find((i) => i.columnField === 'author');
    return item?.value || '';
  });
  const [debouncedAuthor] = useDebounce(author, 300);
  const statusValues = useMemo(() => {
    const item = (filterModel.items || []).find((i) => i.columnField === 'status');
  ...
  useEffect(() => {
    ...
    setFilterModel({ ...(filterModel || { items: [] }), quickFilterValues: values });
  }, [debouncedQuick]);

  // Sync author input to model (debounced)
  useEffect(() => {
    const next = debouncedAuthor.trim();
    const otherItems = (filterModel.items || []).filter((i) => i.columnField !== 'author');
    const newItems = next
      ? otherItems.concat([{ columnField: 'author', operatorValue: 'contains', value: next }])
      : otherItems;
  ...
  const handleStatusChange = useCallback(
    (event) => {
      ...
      const otherItems = (filterModel.items || []).filter((i) => i.columnField !== 'status');
      const newItems = values.length
        ? otherItems.concat([{ columnField: 'status', operatorValue: 'isAnyOf', value: values }])
        : otherItems;
      setFilterModel({ ...filterModel, items: newItems });
    },
  ...
  const statusToText = useCallback((s) => {
    ...
    <Box display="flex" gridGap={8} flexWrap="wrap" alignItems="center" mb={2}>
      ...
      <FormControl variant="outlined" size="small" style={{ minWidth: 200 }}>
        ...
        <Select
          ...
          renderValue={(selected) => (selected || []).map((s) => statusToText(s)).join(', ')}
        >
      ...
      <FormControl variant="outlined" size="small" style={{ minWidth: 160 }}>
        ...
        <Select value={sortField} onChange={handleSortField} label={sortByLabel}>
          <MenuItem value="created">{language === 'en' ? 'Last Edited' : 'Dernière modification'}</MenuItem>
          <MenuItem value="title">{language === 'en' ? 'Title' : 'Titre'}</MenuItem>
          <MenuItem value="author">{language === 'en' ? 'Author' : 'Auteur'}</MenuItem>
          <MenuItem value="progress">{language === 'en' ? 'Progress' : 'Progrès'}</MenuItem>
        </Select>
      </FormControl>
      ...
      <FormControl variant="outlined" size="small" style={{ minWidth: 140 }}>
        ...
        <Select value={sortDir} onChange={handleSortDir} label={directionLabel}>
          <MenuItem value="asc">{language === 'en' ? 'Ascending' : 'Croissant'}</MenuItem>
          <MenuItem value="desc">{language === 'en' ? 'Descending' : 'Décroissant'}</MenuItem>
        </Select>
      </FormControl>
