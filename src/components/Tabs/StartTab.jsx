...
const { DataCollectionSampling, ...filtereMetadataScopeCodes } =
  metadataScopeCodes;
const StartTab = ({
  disabled,
  record,
  ...
  const updateResourceType = (value) => {
    if (isOnlyOther(value)) {
      const variableIds = getRecordEssentialVariables(record);
      if (!variableIds.includes("other")) {
        const nextVariableIds = [...variableIds, "other"];
        updateRecord("essentialVariables")(nextVariableIds);
        updateRecord("eov")(getLegacyEovs(nextVariableIds));
      }
    }
