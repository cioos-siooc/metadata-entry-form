...
const Reviewer = () => {
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = onAuthStateChanged(getAuth(firebase), (authUser) => {
      if (authUser) {
        ...
        onValue(usersRef, (regionUsersRaw) => {
          const loadedRecords = loadRegionRecords(regionUsersRaw, ['', 'submitted', 'published']);
  ...
  // Toggle modal helper
  const toggleModal = useCallback((modalSetter, state, key = '', userID = '') => {
    setModalKey(key);
  ...
  const handleDeleteRecord = useCallback((recordID, userID) => {
  ...
  const handleTransferRecord = useCallback((recordID, userID) => {
  ...
  return (
    <Grid container direction="column" justifyContent="space-between" alignItems="stretch" spacing={3}>
      ...
      <GitHubPublishDialog
        ...
        region={region}
        recordTitle={records.find((r) => r.recordID === modalKey)?.title?.[language] || ''}
      ...
      >
        <Alert onClose={closeToast} severity={toastSeverity} variant="filled" elevation={6}>
          {toastMessage}
        </Alert>
      </Snackbar>
      ...
      <Grid item xs>
        ...
        <Typography variant="body2" color="textSecondary" style={{ marginTop: '8px' }}>
          <I18n>
            <En>
              Review, manage, and publish metadata records. Use filters to find specific submissions
              by status, author, or title.
            </En>
            <Fr>
              Examinez, gérez et publiez les enregistrements de métadonnées. Utilisez les filtres
              pour trouver des soumissions spécifiques par statut, auteur ou titre.
            </Fr>
          </I18n>
        </Typography>
      </Grid>
