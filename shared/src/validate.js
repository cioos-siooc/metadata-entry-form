...
// Exported so clients can drive UI from validation state — the mobile app's
// completeness ledger groups sections by `validators[field].tab`.
export const validators = {
  ...
  resourceType: {
    // Length-checked, not bare truthiness: blankRecord now declares
    // `resourceType: []`, and `[]` is truthy. Behaviour is unchanged — an
    // unset and an empty theme list both fail, as they always did.
    validation: (val) => Boolean(val && val.length),
    tab: "start",
    error: {
  ...
  map: {
    ...
    validation: (val, record) => {
      ...
      return (
        ...
        // "no theme picked yet" — length-checked because blankRecord now
        // declares `resourceType: []`, and `![]` is false. Without this the
        // spatial section would become required on a brand-new record.
        !record.resourceType?.length ||
        (Array.isArray(record.resourceType) &&
          record.resourceType.includes("biological") &&
          description)
      );
    },
  },
  ...
  taxa: {
    tab: "taxa",
    // Length-checked for the same reason as `resourceType`: a record with an
    // empty taxa array genuinely has no taxonomic coverage. `noTaxa` remains
    // the explicit escape hatch.
    validation: (val, record) => Boolean(record.noTaxa || (val && val.length)),
    error: {
