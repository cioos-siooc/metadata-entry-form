...
async function assertReviewerOrAdmin(context, region, actionName) {
  ...
  try {
    ...
    const allowed =
      admins.includes(normalizedEmail) || reviewers.includes(normalizedEmail);
    ...
    functions.logger.error(
      `[authz] Failed to verify permissions for region ${region}:`,
      error,
    );
    throw new functions.https.HttpsError(
      "internal",
...
async function getBaseUrl(region) {
  try {
    ...
    functions.logger.error(
      `Error fetching DataCite API domain for region ${region}:`,
      error,
    );
...
exports.createDraftDoi = functions.https.onCall(async (data, context) => {
  ...
  functions.logger.info("[createDraftDoi] Called", {
    region,
    recordKeys: record ? Object.keys(record) : null,
    type: record?.data?.type,
    prefix: record?.data?.attributes?.prefix,
  });
  ...
  try {
    ...
    functions.logger.error(
      `Error fetching Datacite Auth Hash for region ${region}:`,
      error,
    );
    return null;
  }

  functions.logger.info("[createDraftDoi] authHash", {
    present: !!authHash,
    length: authHash?.length,
  });

  try {
    const baseUrl = await getBaseUrl(region);
    functions.logger.info(
      "[createDraftDoi] POSTing to:",
      baseUrl,
      "body:",
    ...
    functions.logger.info(
      "[createDraftDoi] Success! Response status:",
      response.status,
    );
    return response.data;
  } catch (err) {
    functions.logger.error("[createDraftDoi] DataCite API error", {
...
exports.getDoiStatus = functions.https.onCall(async (data) => {
  ...
  try {
    ...
    functions.logger.error(
      `Error fetching Datacite Prefix for region ${data.region}:`,
      error,
    );
    return null;
  }
  ...
  try {
    ...
    functions.logger.error(
      `Error fetching Datacite Auth Hash for region ${data.region}:`,
      error,
    );
    return null;
  }
...
exports.testDataciteCredentials = functions.https.onCall(async (data) => {
  ...
  if (!authHash || !prefix) {
    try {
      ...
      if (!authHash) {
        authHash = (
          await credentialsRef.child("dataciteHash").once("value")
        ).val();
      }
      if (!prefix) {
      ...
      functions.logger.error(
        `[testDataciteCredentials] Error fetching credentials for region ${region}:`,
        error,
      );
      throw new functions.https.HttpsError(
        "internal",
  ...
  const apiDomain =
    typeof data === "object" && data.apiDomain ? data.apiDomain : null;
  const baseUrl = apiDomain
    ? API_DOMAINS[apiDomain] || API_DOMAINS.production
    : await getBaseUrl(region);
  let testDoi;
  ...
  try {
    ...
    functions.logger.info(
      `[testDataciteCredentials] Draft DOI created: ${testDoi}`,
    );
  } catch (err) {
    functions.logger.error("[testDataciteCredentials] Create failed:", {
      status: err.response?.status,
      data: err.response?.data,
    });
    if (err.response && err.response.status === 401) {
  ...
  if (testDoi) {
    try {
      ...
      functions.logger.info(
        `[testDataciteCredentials] Test DOI ${testDoi} deleted.`,
      );
    } catch (deleteErr) {
      functions.logger.warn(
        `[testDataciteCredentials] Failed to delete test DOI ${testDoi}:`,
        deleteErr.message,
      );
...
exports.getCredentialsStored = functions.https.onCall(async (data) => {
  try {
    const credentialsRef = admin
      ...
      .child(data)
      .child("dataciteCredentials");
    const authHashSnapshot = await credentialsRef
      .child("dataciteHash")
      .once("value");
...
exports.getDatacitePrefix = functions.https.onCall(async (region) => {
  try {
    ...
    throw new Error(
      `Error fetching Datacite Prefix for region ${region}: ${error}`,
    );
  }
});
...
async function transitionDoiState(doi, region, event) {
  ...
  try {
    ...
    functions.logger.error(
      `[transitionDoiState] Error fetching auth hash for region ${region}:`,
      error,
    );
    throw new functions.https.HttpsError(
      "internal",
      "Failed to read DataCite credentials.",
    );
  }
  ...
  try {
    ...
    functions.logger.info(
      `[transitionDoiState] DOI ${doi} transitioned via event "${event}" → state: ${newState}`,
    );
    return newState;
  } catch (err) {
    functions.logger.error(
      `[transitionDoiState] Error for DOI ${doi}, event "${event}":`,
...
exports.publishDoi = functions.https.onCall(
  async ({ doi, region }, context) => {
...
// Valid from: draft.
exports.registerDoi = functions.https.onCall(
  async ({ doi, region }, context) => {
    await assertReviewerOrAdmin(context, region, "change DOI status");
