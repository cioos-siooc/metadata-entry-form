...
async function checkPermissions(email, region) {
  ...
  if (!admins.includes(email) && !reviewers.includes(email)) {
    throw new HttpsError('permission-denied', 'User must be an admin or reviewer.');
  }
}

exports.githubPublishRecord = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'The function must be called while authenticated.');
  }

  const { region, files, commitMessage } = request.data;

  if (!region || !files || !Array.isArray(files) || files.length === 0) {
      throw new HttpsError('invalid-argument', 'Missing required parameters: region and files.');
  }

  // 1. Check Permissions
  await checkPermissions(request.auth.token.email, region);

  // 2. Fetch GitHub Config
  const configSnapshot = await admin.database().ref(`admin/${region}/githubCredentials`).once('value');
  const config = configSnapshot.val();
  if (!config || !config.token) {
    throw new HttpsError('failed-precondition', 'GitHub configuration missing.');
  }
  ...
  try {
      ...
      throw new HttpsError('internal', `GitHub API Error: ${error.message}`);
  }
});
