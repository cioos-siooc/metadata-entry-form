...
async function checkPermissions(email, region) {
  const permissionsSnapshot = await admin
    .database()
    .ref(`admin/${region}/permissions`)
    .once("value");
  const permissions = permissionsSnapshot.val();
  const admins =
    permissions && permissions.admins
      ? permissions.admins.split(",").map((e) => e.trim())
      : [];
  const reviewers =
    permissions && permissions.reviewers
      ? permissions.reviewers.split(",").map((e) => e.trim())
      : [];
...
exports.githubPublishRecord = functions.https.onCall(async (data, context) => {
  ...
  if (!region || !files || !Array.isArray(files) || files.length === 0) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Missing required parameters: region and files.",
    );
  }
