...
async function adminRoutes(app) {
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      if (!Array.isArray(admins) || !Array.isArray(reviewers)) {
        return reply
          .code(422)
          .send({ error: "admins and reviewers arrays required" });
      }

      await withTransaction(async (client) => {
        await client.query("DELETE FROM region_permissions WHERE region = $1", [
          request.region,
        ]);
        const insert =
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      if (!Array.isArray(projects))
        return reply.code(422).send({ error: "projects array required" });

      await withTransaction(async (client) => {
        await client.query("DELETE FROM region_projects WHERE region = $1", [
          request.region,
        ]);
  ...
  app.get(
    "/regions/:region/admin/datacite-credentials",
    adminGuard,
    async (request) => {
      const result = await query(
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const config = {
        ...
        ...(prefix && { prefix }),
        ...(apiDomain && { apiDomain }),
        ...(Array.isArray(doiSuffixModes) &&
          doiSuffixModes.length && { doiSuffixModes }),
        ...(doiStatusManagement && { doiStatusManagement }),
      };
      config.apiDomain = config.apiDomain || "api.datacite.org";
      const secretEnc = dataciteHash
      ...
      if (!config.prefix || !secretEnc) {
        return reply
          .code(422)
          .send({ error: "prefix and dataciteHash required" });
      }
      await query(
  ...
  app.get(
    "/regions/:region/admin/github-credentials",
    memberGuard,
    async (request, reply) => {
  ...
  app.put(
    ...
    adminGuard,
    async (request, reply) => {
      const { owner, repo, branch, environment, fileTemplate, token } =
        request.body || {};
      if (!owner || !repo)
        return reply.code(422).send({ error: "owner and repo required" });
      ...
      const secretEnc = token
        ? encryptSecret(token)
        : (existing.rows[0]?.secret_enc ?? null);
  ...
  app.get(
    "/regions/:region/admin/record-generator-url",
    adminGuard,
    async (request) => {
      const result = await query(
  ...
  app.put(
    ...
    async (request) => {
      await query(
        ...
        [request.region, request.body?.url || null],
      );
