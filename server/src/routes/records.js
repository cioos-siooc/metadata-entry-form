...
async function loadPendingShares(recordId) {
  const result = await query(
    "SELECT email FROM record_share_invites WHERE record_id = $1",
    [recordId],
  );
  if (!result.rows.length) return null;
...
async function recordRoutes(app) {
  ...
  app.get(
    "/regions/:region/records/shared-with-me",
    guarded,
    async (request) => {
      const result = await query(
        `SELECT r.*, u.email AS owner_email, u.display_name AS owner_name
  ...
  app.put("/regions/:region/records/:id", guarded, async (request, reply) => {
    ...
    if (
      expected &&
      new Date(expected).getTime() !== new Date(row.updated_at).getTime()
    ) {
      return reply
        .code(409)
        .send({ error: "Record was changed by someone else" });
    }
    ...
    if (
      ...
      return reply
        .code(403)
        .send({ error: "Only reviewers can publish records" });
    }
  ...
  app.put(
    "/regions/:region/records/:id/status",
    guarded,
    async (request, reply) => {
      const row = await loadRecordRow(request.region, request.params.id);
      if (!row) return reply.code(404).send({ error: "Record not found" });
      if (!(await canWriteRecord(request, row))) {
        return reply
          .code(403)
          .send({ error: "Not allowed to edit this record" });
      }
      ...
      if (!filename) {
        ...
        if (record.title?.[record.language])
      ...
      if (updated.status !== row.status) {
        fireRecordChange(request.log, {
          region: request.region,
          record: toApi(updated, {
            userinfo: await userinfoFor(updated.user_id),
          }),
          before: { status: row.status },
          after: { status: updated.status },
          kind: "update",
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      if (!(await canWriteRecord(request, row))) {
        return reply
          .code(403)
          .send({ error: "Not allowed to delete this record" });
      }
  ...
  app.post(
    "/regions/:region/records/:id/clone",
    guarded,
    async (request, reply) => {
      const row = await loadRecordRow(request.region, request.params.id);
      if (!row) return reply.code(404).send({ error: "Record not found" });
  ...
  app.post(
    "/regions/:region/records/:id/transfer",
    guarded,
    async (request, reply) => {
      const row = await loadRecordRow(request.region, request.params.id);
      if (!row) return reply.code(404).send({ error: "Record not found" });
      if (
        ...
        return reply
          .code(403)
          .send({ error: "Not allowed to transfer this record" });
      }
      ...
      if (!destination.rows.length) {
        return reply
          .code(404)
          .send({ error: `No user with email ${email} in this region` });
      }
      ...
      await query(
        "DELETE FROM record_shares WHERE record_id = $1 AND user_id = $2",
        [row.id, destination.rows[0].id],
      );
      const updated = result.rows[0];
  ...
  app.put(
    "/regions/:region/records/:id/shares",
    guarded,
    async (request, reply) => {
      const row = await loadRecordRow(request.region, request.params.id);
      if (!row) return reply.code(404).send({ error: "Record not found" });
      if (
        ...
        return reply
          .code(403)
          .send({ error: "Not allowed to share this record" });
      }

      const userIds = request.body?.userIds;
      if (!Array.isArray(userIds))
        return reply.code(422).send({ error: "userIds array required" });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (email === owner?.email?.toLowerCase()) {
        return reply
          .code(422)
          .send({ error: "You cannot share a record with its owner." });
      }
      ...
      if (
        ...
        return reply.code(429).send({
          error: `A record can be shared with at most ${MAX_SHARES_PER_RECORD} people.`,
        });
      }
      ...
      if (user) {
        ...
        return {
          status: "shared",
          email,
          emailSent: await send(mailOptionsRecordShared(...mailArgs)),
        };
      }
      ...
      return {
        status: "invited",
        email,
        emailSent: await send(mailOptionsShareInvitation(...mailArgs)),
      };
    },
  );
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      if (inviteKey) {
        await query(
          "DELETE FROM record_share_invites WHERE record_id = $1 AND email = $2",
          [row.id, String(inviteKey).toLowerCase()],
        );
        return { status: "invite-withdrawn" };
      }
      return reply
        .code(422)
        .send({ error: "Either uid or inviteKey is required." });
    },
  );
}
