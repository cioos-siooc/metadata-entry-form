...
async function formTypeRoutes(app) {
  ...
  app.get("/regions/:region/form-types", memberGuard, async (request) => {
    const includeDisabled =
      request.query.includeDisabled === "1" && request.roles.isAdmin;
  ...
  app.get(
    "/regions/:region/form-types/:id",
    memberGuard,
    async (request, reply) => {
      const result = await query(
        "SELECT * FROM form_types WHERE region = $1 AND id = $2",
  ...
  app.post(
    "/regions/:region/form-types",
    adminGuard,
    async (request, reply) => {
      const { slug, title, description, jsonSchema, uiSchema, enabled } =
        request.body || {};
      ...
      if (!isPlainObject(title))
        return reply.code(422).send({ error: "title object required" });
      ...
      if (!inserted.rows.length) {
        return reply
          .code(409)
          .send({ error: `Form type ${slug} already exists in this region` });
      }
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const { title, description, jsonSchema, uiSchema, enabled } =
        request.body || {};
      ...
      const schemaChanged =
        ...
        JSON.stringify(jsonSchema) !== JSON.stringify(row.json_schema);
  ...
  app.delete(
    ...
    async (request, reply) => {
      const used = await query(
        "SELECT 1 FROM form_submissions WHERE form_type_id = $1 LIMIT 1",
        [request.params.id],
      );
      if (used.rows.length) {
        return reply.code(409).send({
          error: "Form type has submissions; disable it instead of deleting",
        });
      }
