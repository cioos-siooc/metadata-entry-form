...
-- Region display config (shape of a src/regions.js entry) so regions can be
-- created and edited at runtime instead of shipping a frontend code change.
ALTER TABLE regions ADD COLUMN config jsonb NOT NULL DEFAULT '{}';

-- Backfill copied verbatim from src/regions.js. isRA marks the featured
-- Regional Associations on the region selector.
UPDATE regions SET config = $json$
{
...
$json$::jsonb WHERE id = 'test';

-- hakai exists in the DB seed but never had a src/regions.js entry
UPDATE regions SET config = $json$
{
  "title": { "en": "Hakai", "fr": "Hakai" },
  "showInRegionSelector": false
}
$json$::jsonb WHERE id = 'hakai';

-- Down Migration

ALTER TABLE regions DROP COLUMN config;
